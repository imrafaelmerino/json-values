...
public class TestJsArray {
  ...
  public void test_create_one_element_json_array() {
    ...
    JsArray arr1 = arr.prepend(JsStr.of("a"), JsInt.of(10));
    Assertions.assertNotEquals(arr,
      arr1
    );
  ...
  public void testPrepend() {
    ...
    JsArray a = empty.prepend(JsStr.of("a"), JsInt.of(1));
    Assertions.assertEquals(JsInt.of(1), a.last());
    Assertions.assertEquals(JsStr.of("a"), a.head());
    JsArray b = a.prependAll(JsArray.of(-1, -2));
    Assertions.assertEquals(JsArray.of(JsInt.of(-1), JsInt.of(-2), JsStr.of("a"), JsInt.of(1)), b);
  ...
  public void test() {
    ...
    JsArray a = JsArray.of(1, 2, 3, 4);

    Function<JsPair, String> toUpperCase = p -> p.path.last().asKey().name.toUpperCase();
    Assertions.assertEquals(a, a.mapKeys(toUpperCase));

    JsArray b = JsArray.of(JsObj.of("a", JsInt.of(1),
      "b", JsInt.of(2)
      ),
      JsArray.of(JsStr.of("a"), JsObj.of("c", TRUE,
        "d", FALSE))
    );

    Assertions.assertEquals(b, b.mapKeys(toUpperCase));

    JsArray c = b.mapAllKeys(toUpperCase);
    System.out.println(c);

    Assertions.assertTrue(c.streamAll().filter(p -> p.path.last().isKey())
                                       .map(it -> it.path.last().asKey().name)
                                       .allMatch(key -> key.toUpperCase().equals(key)));
    ;
  }
