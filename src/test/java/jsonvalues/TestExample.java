...
public class TestExample {
  ...
  public void test() {
    ...
    JsObjSpec spec = JsObjSpec.strict("name", JsSpecs.str,
      "surname", JsSpecs.str.optional(),
      "languages",JsSpecs.arrayOfStr,
      "age", JsSpecs.integer.optional(),
      "address", JsObjSpec.strict("street", JsSpecs.str,
        "number", JsSpecs.any(it -> it.isInt() || it.isStr()),
        "city", JsSpecs.str.nullable(),
        "coordinates", JsSpecs.tuple(JsSpecs.decimal,
          JsSpecs.decimal)
      )
    );
    ...
    JsObj person = JsObj.of("name", JsStr.of("Rafael"),
      ...
      "languages",JsArray.of("Haskell","Java","Clojure","Scala","Erlang","Prolog"),
      "age", JsInt.of(37),
    ...
    Assertions.assertTrue(spec.test(person).isEmpty());

    Assertions.assertEquals(person,parser.parse(person.toPrettyString()));
    ...
    Assertions.assertEquals(ageOpt.get.apply(newPerson ),ageOpt.get.apply(person).map(i->i+1));
  ...
  public void test_gen() {
    JsObjGen gen = JsObjGen.of("name", JsGens.alphabetic,
      "surname", JsGens.alphabetic.optional(),
      "age", JsGens.choose(16, 100).optional(),
      "address", JsObjGen.of("street", JsGens.alphabetic,
        "number", JsGens.oneOf(JsGens.choose(0, 1000),
          JsGens.alphanumeric
        ),
        "city", JsGens.alphabetic.nullable(),
        "coordinates", JsGens.tuple(JsGens.decimal,
          JsGens.decimal
        )
      )
    );

    JsObjFuture future = JsObjFuture.of("name", () -> completedFuture(JsStr.of("a")),
      "surname", () -> completedFuture(JsObj.empty()),
      "age", () -> completedFuture(JsInt.of(1)),
      "address", JsObjFuture.of("street", () -> completedFuture(JsStr.of("b")),
        "number", () -> completedFuture(JsStr.of("1")),
        "coordinates", JsArrayFuture.of(() -> completedFuture(JsDouble.of(1.5)),
          () -> completedFuture(JsDouble.of(1.5))
        )
      )
    );

    JsObjIO consoleIO = JsObjIO.of("name", JsIOs.read(JsSpecs.str),
      "surname", JsIOs.read(JsSpecs.str),
      "age", JsIOs.read(JsSpecs.integer),
      "address", JsObjIO.of("street", JsIOs.read(JsSpecs.str),
        "number", JsIOs.read(JsSpecs.str),
        "coordinates", JsArrayIO.of(JsIOs.read(JsSpecs.decimal),
