...
public class SpecToGenTest {


  @Test
  public void shouldPrimitivesWithoutConstraints() {
  ...
  @Test
  public void shouldPrimitivesWithConstraints() {
  ...
  @Test
  public void shouldNullablePrimitivesWithoutConstraints() {
  ...
  @Test
  public void shouldArrayOfPrimitivesWithoutConstraints() {
  ...
  @Test
  public void shouldArrayOfPrimitivesWithConstraints() {
  ...
  @Test
  public void shouldNullableArrayOfPrimitivesWithoutConstraints() {
  ...
  @Test
  public void shouldNullableMaps() {
    JsObjSpec spec =
        JsObjSpec.of("a",
                     JsSpecs.mapOfStr()
                            .nullable(),
  ...
  @Test
  public void shouldCustomOptions() {
    JsObjSpec spec =
        JsObjSpec.of("name",
                     str(),
                     "languages",
  ...
  @Test
  public void shouldErrorsConstraints() {
    var spec = JsObjSpec.of("a",
                            JsSpecs.str(StrSchema.withMinLength(10)
                                                 .setMaxLength(15)
                                                 .setPattern("^[a-z]+$")
  ...
  @Test
  public void shouldUnsupportedAnyPredicateWithoutOverride() {
    JsObjSpec spec = JsObjSpec.of("a",
                                  JsSpecs.any(v -> true));
    ...
    Assertions.assertTrue(exception.getMessage()
                                   .contains("AnySuchThat"));
  }

  @Test
  public void shouldUnsupportedObjPredicateWithoutOverride() {
    JsObjSpec spec = JsObjSpec.of("a",
                                  JsSpecs.obj(obj -> true));
    ...
    Assertions.assertTrue(exception.getMessage()
                                   .contains("JsObjSuchThat"));
  }

  @Test
  public void shouldRegexStringRequiresOverride() {
    JsObjSpec spec = JsObjSpec.of("a",
                                  JsSpecs.str(StrSchema.withLength(1,
                                                                   4)
                                                       .setPattern("^[0-9]+$")));
  ...
  @Test
  public void shouldRegexStringWithOverride() {
    JsObjSpec spec = JsObjSpec.of("a",
                                  JsSpecs.str(StrSchema.withLength(1,
                                                                   4)
                                                       .setPattern("^[0-9]+$")));

    JsObjGen gen = SpecToGen.DEFAULT.convert(spec,
                                             Map.of(JsPath.path("/a"),
                                                    JsStrGen.digits(1,
                                                                    4)));

    gen.sample(100)
       .forEach(json -> Assertions.assertTrue(spec.test(json)
                                                  .isEmpty()));
  }

  @Test
  public void shouldDirectConvertOfAnyNamedAndOneOfSpecs() {
    Gen<JsValue> anyGen = SpecToGen.DEFAULT.convert(JsSpecs.any());
    anyGen.sample(50)
          .forEach(v -> Assertions.assertFalse(v == null));

    String name = "named_gen_" + System.nanoTime();
    JsSpec named = JsSpecs.ofNamedSpec(name,
                                       JsObjSpec.of("a",
                                                    JsSpecs.integer()));
    Gen<JsValue> namedGen = SpecToGen.DEFAULT.convert(named);
    namedGen.sample(50)
            .forEach(v -> Assertions.assertTrue(named.test(v)
                                                    .isEmpty()));

    JsSpec oneOf = JsSpecs.oneSpecOf(JsSpecs.str(),
                                     JsSpecs.integer());
    Gen<JsValue> oneOfGen = SpecToGen.DEFAULT.convert(oneOf);
    oneOfGen.sample(100)
            .forEach(v -> Assertions.assertTrue(v.isStr() || v.isInt()));
  }

  @Test
  public void shouldGenerateArraysOfTestedAndBinaryMaps() {
    List<JsSpec> specs = List.of(JsSpecs.arrayOfInt(i -> true),
                                 JsSpecs.arrayOfLong(i -> true),
                                 JsSpecs.arrayOfDouble(i -> true),
                                 JsSpecs.arrayOfBigInt(i -> true),
                                 JsSpecs.arrayOfDec(i -> true),
                                 JsSpecs.arrayOfStr(s -> true));

    specs.forEach(spec -> {
      Gen<JsValue> gen = SpecToGen.DEFAULT.convert(spec);
      Assertions.assertNotNull(gen);
    });

    JsSpec mapSpec = JsSpecs.mapOfBinary();
    Gen<JsValue> mapGen = SpecToGen.DEFAULT.convert(mapSpec);
    mapGen.sample(40)
          .forEach(v -> {
            Assertions.assertTrue(mapSpec.test(v)
                                         .isEmpty());
            v.toJsObj()
             .stream()
             .map(e -> e.value())
             .forEach(bin -> Assertions.assertDoesNotThrow(() -> Base64.getDecoder()
                                                                             .decode(bin.toJsBinary()
                                                                                        .toString())));
          });
  }

  @Test
  public void shouldGenerateSuchThatPrimitiveSpecs() {
    JsObjSpec spec = JsObjSpec.of("a",
                                  JsSpecs.decimal(n -> true),
                                  "b",
                                  JsSpecs.bigInteger(n -> true),
                                  "c",
                                  JsSpecs.doubleNumber(n -> true),
                                  "d",
                                  JsSpecs.str(s -> true),
                                  "e",
                                  JsSpecs.integer(n -> true),
                                  "f",
                                  JsSpecs.longInteger(n -> true),
                                  "g",
                                  JsSpecs.instant(i -> true),
                                  "h",
                                  JsSpecs.binary(b -> true),
                                  "i",
                                  JsSpecs.array()
                                 );
  ...
  @Test
  public void shouldUnsupportedArraySuchThatWithoutOverride() {
    JsObjSpec spec = JsObjSpec.of("a",
                                  JsSpecs.arraySuchThat((JsArray a) -> true));

    IllegalArgumentException exception =
        Assertions.assertThrows(IllegalArgumentException.class,
                                () -> SpecToGen.DEFAULT.convert(spec));
    Assertions.assertTrue(exception.getMessage()
                                   .contains("JsArraySuchThat"));
  }
