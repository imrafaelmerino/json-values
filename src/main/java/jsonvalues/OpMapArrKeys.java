                            ...
                            )
    {
        return json.ifEmptyElse(Trampoline.done(json),
                                (head, tail) ->
                                {
                                    ...
                                    final Trampoline<JsArray> tailCall = Trampoline.more(() -> new OpMapArrKeys(tail).map_(fn,
                                                                                                                           predicate,
                                                                                                                           headPath
                                                                                                                          ));
                                    return ifJsonElse(headObj -> more(() -> tailCall).flatMap(tailResult -> new OpMapObjKeys(headObj).map_(fn,
                                                                                                                                           predicate,
                                                                                                                                           headPath
                                                                                                                                          )
                                                                                                                                     .map(tailResult::prepend)),
                                                      headArr -> more(() -> tailCall).flatMap(tailResult -> new OpMapArrKeys(headArr).map_(fn,
                                                                                                                                           predicate,
                                                                                                                                           headPath.index(-1)
                                                                                                                                          )
                                                                                                                                     .map(tailResult::prepend)),
