...
/**
 * Represents an array of bytes in a JSON-like data structure. The purpose of the {@code JsBinary} class is to allow the
 * inclusion of binary data within JSON-like objects, even though binary data is not part of the JSON specification.
 * This class provides a means to serialize binary data into a string using the Base64 encoding scheme, making it
 * compatible with JSON-like structures.
 *
 * <p>Binary data is often encountered in various data formats, and while JSON itself does not support binary
 * data, the {@code JsBinary} class serves as a custom solution for embedding binary content within JSON-like objects.
 *
 * <p>When serialized to a JSON string, a {@code JsBinary} object is represented as a string containing the
 * Base64-encoded binary data. Consequently, a {@code JsBinary} object and a {@code JsStr} object created from the
 * Base64-encoded string are considered equal if the encoded content matches.
 *
 ...
 * structures, it is not part of the JSON standard. Its purpose is to provide a means of encoding binary data within a
 * format that can coexist with JSON-like data structures.
 *
 * @see JsPrimitive
 */
public final class JsBinary extends JsPrimitive {
  ...
  public static JsBinary of(final String base64) {
    return new JsBinary(requireNonNull(Base64.getDecoder()
                                             .decode(base64)));
  }
  ...
  @Override
  public int hashCode() {
    return Objects.hashCode(Base64.getEncoder()
                                  .encodeToString(value));
  }
}
