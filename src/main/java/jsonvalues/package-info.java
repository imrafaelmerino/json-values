 ...
 * data in a type-safe and structured manner. It includes representations for various JSON data types, such as objects,
 * arrays, primitives, and more.
 * <p>
 * JSON Data Types:
 * <p>
 * - {@link jsonvalues.JsValue}: An interface representing a JSON value, which can be one of the following types:
 * object, array, primitive, null, or nothing. It serves as the root interface for all JSON data types.
 * <p>
 * - {@link jsonvalues.JsObj}: A class representing a JSON object (also known as a map or dictionary). It maps string
 * keys to JSON values.
 * <p>
 * - {@link jsonvalues.JsArray}: A class representing a JSON array, which is an ordered collection of JSON values.
 * <p>
 * - {@link jsonvalues.JsPrimitive}: An interface representing a JSON primitive value, which can be one of the following
 * types: string, number, boolean, or null. It extends `JsValue`.
 * <p>
 ...
 * <p>
 * - {@link jsonvalues.JsDouble}: A class representing a JSON double-precision floating-point number value. It extends
 * `JsPrimitive`.
 * <p>
 ...
 * <p>
 * - {@link jsonvalues.JsNothing}: A class representing a special JSON value called "nothing," which represents a
 * non-existing or undefined value. It extends `JsPrimitive`.
 * <p>
 * JSON Manipulation and Creation:
 * <p>
 * The classes and interfaces in this package allow you to create, manipulate, and traverse JSON data structures. You
 * can build JSON objects, arrays, and primitives, perform operations on them, and serialize/deserialize them as
 * needed.
 * <p>
