 ...
 * character generators like {@code alphanumeric}, {@code digit}, {@code letter}, and {@code alphabetic}. - String
 * generators like {@code digits}, {@code letters}, {@code alphanumeric}, {@code alphabetic}, {@code arbitrary}, and
 * {@code biased}.
 * <p>
 * The length of the generated strings is distributed uniformly over a specified interval.
 * <p>
 * The biased generator produces, with higher probability, potential problematic values that usually cause more bugs
 * (empty and blank strings, for example).
 * <p>
 * If none of the previous factory methods suit your needs, you can still create a JsStr generator from a string
 * generator and the function map:
 *
 * <pre>{@code
 * import fun.gen.Gen;
 * import jsonvalues.JsStr;
...
public final class JsStrGen implements Gen<JsStr> {
  ...
  private static final Gen<JsStr> alphanumeric = new JsStrGen(StrGen.alphanumeric(1,
                                                                                  1));
  private static final Gen<JsStr> digit = new JsStrGen(StrGen.digits(1,
                                                                     1));
  private static final Gen<JsStr> letter = new JsStrGen(StrGen.letters(1,
                                                                       1));
  private static final Gen<JsStr> alphabetic = new JsStrGen(StrGen.alphabetic(1,
                                                                              1));
  private static final Gen<JsStr> ascii = new JsStrGen(StrGen.ascii(1,
                                                                    1));
  ...
  /**
   * Generates a string made up of alphanumeric characters with a length between {@code minLength} and
   * {@code maxLength}.
   *
   * @param minLength Minimum length of the string (inclusive).
   * @param maxLength Maximum length of the string (inclusive).
  ...
  /**
   * Returns a biased generator that produces, with higher probability, potential problematic values that usually cause
   * more bugs. These values include empty strings (if {@code minLength} is equal to zero), blank strings of length
   * {@code minLength}, blank strings of length {@code maxLength}, arbitrary strings of length {@code minLength}, and
   * arbitrary strings of length {@code maxLength}.
   *
   * @param minLength Minimum length of the string (inclusive).
   * @param maxLength Maximum length of the string (inclusive).
   * @return A string generator.
