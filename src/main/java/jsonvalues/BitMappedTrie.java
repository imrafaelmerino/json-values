...
interface NodeModifier {
  ...
  NodeModifier COPY_NODE = (o, i) -> obj().copy(o,
                                                i + 1);
...
/**
 * A `bit-mapped trie` is a very wide and shallow tree (for integer indices the depth will be `≤6`). Each node has a
 * maximum of `32` children (configurable). Access to a given position is done by converting the index to a base 32
 * number and using each digit to descend the tree. Modifying the tree is done similarly, but along the way the path is
 * copied, returning a new root every time. `Append` inserts in the last leaf, or if the tree is full from the right, it
 * adds another layer on top of it (the old root will be the first of the new one). `Prepend` is done similarly, but an
 * offset is needed, because adding a new top node (where the current root would be the last node of the new root)
...
final class BitMappedTrie<T> {
  ...
  private static final BitMappedTrie<?> EMPTY = new BitMappedTrie<>(obj(),
                                                                    obj().empty(),
                                                                    0,
                                                                    0,
                                                                    0);
  ...
  static int digit(int num,
                   int depthShift) {
    return lastDigit(firstDigit(num,
                                depthShift));
  }
  ...
  static <T> BitMappedTrie<T> ofAll(Object array) {
    ...
    return (size == 0) ? empty() : ofAll(array,
                                         type,
                                         size);
  }

  private static <T> BitMappedTrie<T> ofAll(Object array,
                                            ArrayType<T> type,
                                            int size) {
    int shift = 0;
    for (ArrayType<T> t = type; t.lengthOf(array) > BRANCHING_FACTOR; shift += BRANCHING_BASE) {
      array = t.grouped(array,
                        BRANCHING_FACTOR);
      t = obj();
    }
    return new BitMappedTrie<>(type,
                               array,
                               0,
                               size,
                               shift);
  ...
  private static <T> BitMappedTrie<T> collapsed(ArrayType<T> type,
    ...
    for (; shift > 0; shift -= BRANCHING_BASE) {
      ...
      if (skippedElements != digit(offset,
                                   shift)) {
        break;
      }
      array = obj().getAt(array,
                          skippedElements);
      offset -= treeSize(skippedElements,
                         shift);
    }
    return new BitMappedTrie<>(type,
                               array,
  ...
  BitMappedTrie<T> prependAll(Vector<T> iterable) {
    return prepend(iterable.reverseIterator(),
                   iterable.length());
  }
  ...
  private BitMappedTrie<T> prepend(java.util.Iterator<? extends T> iterator,
    ...
    while (size > 0) {
      ...
      if (result.isFullLeft()) {
        array = obj().copyUpdate(obj().empty(),
                                 BRANCHING_FACTOR - 1,
                                 array);
        shift += BRANCHING_BASE;
        offset = treeSize(BRANCHING_FACTOR - 1,
                          shift);
      }

      final int index = offset - 1;
      final int delta = Math.min(size,
                                 lastDigit(index) + 1);
      size -= delta;

      array = result.modify(array,
                            shift,
                            index,
                            COPY_NODE,
                            prependToLeaf(iterator));
      result = new BitMappedTrie<>(type,
                                   array,
                                   offset - delta,
                                   result.length + delta,
                                   shift);
  ...
  private NodeModifier prependToLeaf(java.util.Iterator<? extends T> iterator) {
    return (array, index) -> {
      final Object copy = type.copy(array,
                                    BRANCHING_FACTOR);
  ...
  private boolean isFullRight() {
    return (offset + length + 1) > treeSize(BRANCHING_FACTOR,
                                            depthShift);
  }
  ...
  BitMappedTrie<T> update(int index,
                          T element) {
    final Object root = modify(array,
                               depthShift,
                               offset + index,
                               COPY_NODE,
                               updateLeafWith(type,
  ...
  private NodeModifier updateLeafWith(ArrayType<T> type,
                                      T element) {
    return (a, i) -> type.copyUpdate(a,
                                     i,
                                     element);
  }
  ...
  BitMappedTrie<T> drop(int n) {
    if (n >= length) {
      ...
      final Object root = arePointingToSameLeaf(0,
                                                n)
                          ? array
                          : modify(array,
                                   depthShift,
  ...
  BitMappedTrie<T> take(int n) {
    if (n <= 0) {
      ...
      final int index = n - 1;
      final Object root = arePointingToSameLeaf(index,
                                                length - 1)
                          ? array
                          : modify(array,
                                   depthShift,
  ...
  private Object modifyNonLeaf(Object root,
                               int depthShift,
                               int index,
                               NodeModifier node,
                               NodeModifier leaf) {
    int previousIndex = firstDigit(index,
                                   depthShift);
    root = node.apply(root,
                      previousIndex);

    Object array = root;
    for (int shift = depthShift - BRANCHING_BASE; shift >= BRANCHING_BASE; shift -= BRANCHING_BASE) {
      final int prev = previousIndex;
      previousIndex = digit(index,
                            shift);
      array = setNewNode(node,
                         prev,
                         array,
                         previousIndex);
    }

    final Object newLeaf = leaf.apply(obj().getAt(array,
                                                  previousIndex),
                                      lastDigit(index));
    obj().setAt(array,
                previousIndex,
  ...
  private Object setNewNode(NodeModifier node,
                            int previousIndex,
                            Object array,
                            int offset) {
    final Object previous = obj().getAt(array,
                                        previousIndex);
    final Object newNode = node.apply(previous,
                                      offset);
    obj().setAt(array,
                previousIndex,
                newNode);
  ...
  T get(int index) {
    ...
    return type.getAt(leaf,
                      leafIndex);
  }

  /**
   * fetch the leaf, corresponding to the given index. Node: the offset and length should be taken into consideration as
   * there may be leading and trailing garbage. Also, the returned array is mutable, but should not be mutated!
   */
  ...
  <T2> void visit(LeafVisitor<T2> visitor) {
    ...
    for (int index = 0; index < length; ) {
      ...
      final int end = getMin(start,
                             index,
                             leaf);

      globalIndex = visitor.visit(globalIndex,
                                  leaf,
                                  start,
                                  end);
  ...
  private <U> int map(Function<? super T, ? extends U> mapper,
                      Object results,
                      int index,
                      T leaf,
                      int start,
