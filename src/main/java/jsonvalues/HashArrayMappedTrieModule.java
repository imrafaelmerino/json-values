...
import static java.lang.Integer.bitCount;
 ...
 interface HashArrayMappedTrieModule {
    ...
    abstract class AbstractNode implements HashArrayMappedTrie {
        ...
        abstract Optional<JsValue> lookup(int shift, int keyHash, String key);

        abstract JsValue lookup(int shift, int keyHash, String key, JsValue defaultJsValuealue);

        abstract AbstractNode modify(int shift, int keyHash, String key, JsValue value, Action action);
        ...
        public Iterator<String> keysIterator() {
            return HashArrayMappedTrie.map(nodes(), LeafNode::key);

        }

        @Override
        public Iterator<JsValue> valuesIterator() {
            return HashArrayMappedTrie.map(nodes(), LeafNode::value);
        }
        ...
        @Override
        public JsValue getOrElse(String key, JsValue value) {
            return lookup(0, Objects.hashCode(key), key, value);
        }
        ...
        @Override
        public HashArrayMappedTrie put(String key, JsValue value) {
    ...
    final class LeafSingleton extends LeafNode  {
        ...
        LeafSingleton(int hash, String key, JsValue value) {
            this.hash = hash;
            this.key = key;
            this.value = value;
        }

        private boolean equals(int keyHash, String key) {
            return keyHash == hash && Objects.equals(key, this.key);
        }

        @Override
        Optional<JsValue> lookup(int shift, int keyHash, String key) {
            return equals(keyHash, key) ? Optional.of(value) : Optional.empty();
        }
    ...
    final class LeafList extends LeafNode implements Serializable {
        ...
        LeafList(int hash, String key, JsValue value, LeafNode tail) {
            this.hash = hash;
            this.key = key;
            this.value = value;
    ...
    final class IndexedNode extends AbstractNode {
        ...
        JsValue lookup(int shift, int keyHash, String key, JsValue defaultJsValuealue) {
            ...
            if ((bitmap & bit) != 0) {
                final AbstractNode n = (AbstractNode) subNodes[fromBitmap(bitmap, bit)];
                return n.lookup(shift + SIZE, keyHash, key, defaultJsValuealue);
            } else {
                return defaultJsValuealue;
            }
        }
