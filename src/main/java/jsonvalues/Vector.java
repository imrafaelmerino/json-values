...
final class Vector<T> implements Iterable<T> {
  ...
  private static <T> Vector<T> ofAll(BitMappedTrie<T> trie) {
    return (Objects.requireNonNull(trie)
                   .length() == 0)
           ? empty()
           : new Vector<>(trie);
  }
  ...
  Vector<T> reverse() {
    return (length() <= 1) ? this : foldLeft(empty(),
                                             Vector::prepend);
  }
  ...
  public Vector<T> update(int index,
                          T element) {
    return wrap(trie.update(index,
                            element));
  }
  ...
  public int hashCode() {
    return hash(this,
                (acc, hash) -> acc * 31 + hash);
  }


  private int hash(Vector<T> iterable,
