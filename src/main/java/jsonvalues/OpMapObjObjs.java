  ...
  Trampoline<JsObj> map(final BiFunction<? super JsPath, ? super JsObj, JsValue> fn,
    ...
    return json.ifEmptyElse(Trampoline.done(json),
      (head, tail) ->
      {
        ...
        final Trampoline<JsObj> tailCall = Trampoline.more(() -> new OpMapObjObjs(tail).map(fn, startingPath));
        return ifObjElse(headObj -> more(() -> tailCall).map(tailResult -> tailResult.set(JsPath.fromKey(head._1),
          fn.apply(JsPair.of(headPath, headObj).path, headObj))),
          headElem -> more(() -> tailCall).map(tailResult -> tailResult.set(JsPath.fromKey(head._1), headElem))).apply(head._2);
      }
    );
  }
    ...
    return json.ifEmptyElse(Trampoline.done(json),
      (head, tail) ->
      {
        ...
        return ifJsonElse(headObj ->
          {
            JsValue headMapped = fn.apply(headPath, headObj);
            if (headMapped.isObj())
