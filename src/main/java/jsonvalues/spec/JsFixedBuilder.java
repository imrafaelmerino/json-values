...
public final class JsFixedBuilder {
    ...
    public static JsFixedBuilder name(String name) {
        if (!isValidName.test(name))
            throw new IllegalArgumentException("The name `%s` of the Fixed binary doesn't follow the pattern `%s`".formatted(name,
                                                                                                                             AVRO_NAME_PATTERN));
    ...
    public JsFixedBuilder namespace(String nameSpace) {
        ...
        if (!isValidNamespace.test(nameSpace))
            throw new IllegalArgumentException("The namespace `%s` of the Fixed binary with name `%s` doesn't follow the pattern `%s`".formatted(nameSpace,
                                                                                                                                                 name,
