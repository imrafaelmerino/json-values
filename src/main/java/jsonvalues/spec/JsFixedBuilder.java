...
public final class JsFixedBuilder {
    ...
    /**
     * Sets the name of the fixed binary specification. The name must follow the Avro naming conventions, adhering to
     * the regex pattern: {@code [A-Za-z_][A-Za-z0-9_]*}.
     *
     ...
     * <p>
     * Aliases provide alternative names for the fixed binary, and they can be used interchangeably when referring to
    ...
    public JsSpec build(final int size) {
        ...
        var metadata = new FixedMetaData(name, nameSpace, aliases,doc);
        var spec = new JsFixedBinary(false, size, metadata);
        JsSpecCache.putAll(metadata.getFullName(), aliases, spec);
        return spec;
