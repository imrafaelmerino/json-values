 ...
 * <p>
 * In both cases JsonWriter writes into a byte[] buffer. If stream is used as target, it will copy buffer into the
...
class JsWriter {
  ...
  private void enlargeOrFlush(int size,
    ...
    if (target != null) {
      try {
        ...
        throw new JsSerializerException("Unable to write to target stream.",
                                        ex);
      }
      position = 0;
      if (padding > buffer.length) {
        buffer = Arrays.copyOf(buffer,
                               buffer.length + buffer.length / 2 + padding);
      }
    } else {
      buffer = Arrays.copyOf(buffer,
                             buffer.length + buffer.length / 2 + padding);
  ...
  private void writeQuotedString(CharSequence str,
                                 int i,
                                 int cur,
                                 int len) {
    ...
    for (; i < len; i++) {
      ...
      if (c == '"') {
        ...
        int cp = Character.codePointAt(str,
                                       i);
  ...
  /**
   * Current position in the buffer. When stream is not used, this is also equivalent to the size of the resulting JSON
   * in bytes
   *
   ...
   * stream as target, meaning new usages of the JsonWriter will try to use the already provided stream. It will not do
   * anything if stream was not used
   * <p>
  ...
  /**
   * Custom objects can be serialized based on the implementation specified through this interface. Annotation processor
   * creates custom deserializers at compile time and registers them into DslJson.
   *
   * @param <T> type
   */
  interface WriteObject<T> {

    void write(JsWriter writer,
               T value);
  }
