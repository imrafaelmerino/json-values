...
public final class JsObjSpecParser {
  ...
  /**
   * Creates a JSON object parser based on the provided JSON object specification (spec). The parser will validate that
   * every field in a JSON object adheres to the schema defined in the given specification.
   *
  ...
  /**
   * Parses an input stream of bytes into a JSON object that must conform to the spec of the parser. If the input stream
   * of bytes doesn't represent a well-formed JSON object or is a well-formed JSON that doesn't conform to the spec of
   * the parser, a ParsingException failure wrapped in a Try computation is returned. Any I/O exception that occurs
   * while processing the input stream is also wrapped in a Try computation.
   *
