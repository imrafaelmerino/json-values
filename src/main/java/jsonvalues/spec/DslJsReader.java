...
final class DslJsReader {
  ...
  /**
   * Bind input stream for processing. Stream will be processed in byte[] chunks. If stream is null, reference to stream
   * will be released.
   *
  ...
  DslJsReader process(InputStream stream) throws JsParserException {
    ...
    this.currentIndex = 0;
    this.stream = stream;
    this.readLimit = Math.min(this.length,
                              bufferLenWithExtraSpace);
    int available;
    try {
      available = readFully(buffer,
                            stream,
                            0);
    } catch (IOException e) {
      throw newParseError(e.getMessage());
    }
    readLimit = Math.min(available,
                         bufferLenWithExtraSpace);
  ...
  /**
   * Bind byte[] buffer for processing. If this method is used in combination with process(InputStream) this buffer will
   * be used for processing chunks of stream. If null is sent for byte[] buffer, new length for valid input will be set
   * for existing buffer.
   *
  ...
  byte read() throws JsParserException {
    ...
    if (currentIndex >= length) {
      throw JsParserException.reasonAt(UNEXPECTED_END_OF_JSON,
                                       currentIndex);
    }
  ...
  /**
   * Which was last byte read from the JSON input. JsonReader doesn't allow to go back, but it remembers previously read
   * byte
   *
  ...
  JsParserException newParseError(String description) {
    return JsParserException.reasonAt(description,
                                      getPositionInStream(0));
  }

  JsParserException newParseError(String description,
                                  int offset) {
    return JsParserException.reasonAt(description,
                                      getPositionInStream(offset));
  }
  ...
  char[] prepareBuffer(int start,
                       int len) throws JsParserException {
    if (len > maxNumberDigits) {
      throw newParseError(ParserErrors.TOO_MANY_DIGITS.formatted(len),
                          len);
    }
  ...
  boolean allWhitespace(int start,
                        int end) {
    byte[] _buf = buffer;
  ...
  String readString() throws JsParserException {
    int len = parseString();
    return valuesCache == null ? new String(chars,
                                            0,
                                            len) : valuesCache.get(chars,
                                                                   len);
  }
  ...
  int parseString() throws JsParserException {
    ...
    if (last != '"') {
      throw newParseError(ParserErrors.EXPECTING_STRING_START);
    } else if (currentIndex == length) {
      throw newParseError(ParserErrors.PREMATURE_END_OF_JSONSTRING);
    }
    ...
    int _tmpLen = Math.min(_tmp.length,
                           remaining);
    int i = 0;
    while (i < _tmpLen) {
      bb = buffer[ci++];
    ...
    if (i == _tmp.length) {
      ...
      _tmp = chars = Arrays.copyOf(chars,
                                   newSize);
    }
    _tmpLen = _tmp.length;
    currentIndex = ci;
  ...
  String readKey() {
    int len = parseString();
    String key = keyCache != null ? keyCache.get(chars,
                                                 len) : new String(chars,
                                                                   0,
                                                                   len);
    if (readNextToken() != ':') {
      throw newParseError(ParserErrors.EXPECTING_COLON);
    }
    readNextToken();
    return key;
  }
  ...
  /**
   * Checks if 'null' value is at current position. This means last read byte was 'n' and 'ull' are next three bytes. If
   * last byte was n but next three are not 'ull' it will throw since that is not a valid JSON construct.
   *
  ...
  /**
   * Checks if 'true' value is at current position. This means last read byte wasn't' and 'rue' are next three bytes. If
   * last byte was t but next three are not 'rue' it will throw since that is not a valid JSON construct.
   *
  ...
  void checkArrayEnd() {
    if (last != ']') {
      if (currentIndex >= length) {
        throw newParseError(ParserErrors.UNEXPECTED_END_OF_ARRAY);
      }
