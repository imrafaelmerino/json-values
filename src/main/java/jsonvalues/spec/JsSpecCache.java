...
final class JsSpecCache {
    ...
    static void put(String name, JsSpec spec) {
        synchronized (JsSpecCache.class) {
            validateDoesntExist(requireNonNull(name));
            cache.put(requireNonNull(name), requireNonNull(spec));
        }
    }

    static void putAll(String name, List<String> aliases, JsSpec spec) {
        synchronized (JsSpecCache.class) {
            validateDoesntExist(requireNonNull(name));
