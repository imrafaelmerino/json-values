...
/**
 * Configuration for DslJson options. By default, key cache is enabled. Everything else is not configured. To load
 * `META-INF/services` call `includeServiceLoader()`
 */
class Settings {
   ...
   * <p>
   * Not every "JSON string" will use this cache... eg UUID, LocalDate don't create an instance of string and therefore
  ...
  /**
   * Specify maximum allowed size for digits buffer. Default is 512. Digits buffer is used when processing strange/large
   * input numbers.
   *
  ...
  /**
   * Specify maximum allowed size for string buffer. Default is 128MB To protect against malicious inputs, maximum
   * allowed string buffer can be reduced.
   *
